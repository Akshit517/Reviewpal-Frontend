...
class ChannelMemberBloc extends Bloc<ChannelMemberEvent, ChannelMemberState> {
  ...
  void _getChannelMembers(
      GetChannelMembersEvent event, Emitter<ChannelMemberState> emit) async {
    emit(ChannelMemberLoading());
    final result = await getChannelMembersUseCase(ChannelMemberParams(
        workspaceId: event.workspaceId,
        categoryId: event.categoryId,
        channelId: event.channelId));
    result.fold((failure) => emit(ChannelMemberError(message: failure.message)),
        (members) {
      final membersByTeam = <Team, List<ChannelMember>>{};
      for (final member in members) {
        final team = member.team;
        if (!membersByTeam.containsKey(team)) {
          membersByTeam[team!] = [];
        }
        membersByTeam[team]!.add(member);
      }
      emit(ChannelMemberSuccess(membersByTeam: membersByTeam));
    });
  }

  void _addChannelMember(
      AddChannelMemberEvent event, Emitter<ChannelMemberState> emit) async {
    emit(ChannelMemberLoading());
    final result = await addChannelMemberUseCase(ChannelMemberParams(
        workspaceId: event.workspaceId,
        categoryId: event.categoryId,
        channelId: event.channelId,
        team: event.team,
  ...
  void _deleteChannelMember(
      RemoveChannelMemberEvent event, Emitter<ChannelMemberState> emit) async {
    emit(ChannelMemberLoading());
    final result = await deleteChannelMemberUseCase(ChannelMemberParams(
        workspaceId: event.workspaceId,
        categoryId: event.categoryId,
        channelId: event.channelId,
        email: event.userEmail));
  ...
  void _updateChannelMember(
      UpdateChannelMemberEvent event, Emitter<ChannelMemberState> emit) async {
    emit(ChannelMemberLoading());
    final result = await updateChannelMemberUseCase(ChannelMemberParams(
        workspaceId: event.workspaceId,
        categoryId: event.categoryId,
        channelId: event.channelId,
        email: event.userEmail,
